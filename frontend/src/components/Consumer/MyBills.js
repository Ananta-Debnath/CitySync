...
const PayModal = ({ bill, onClose, onSuccess, t, isDark }) => {
  ...
  const handlePay = async () => {
    ...
    try {
      const res  = await authFetch('/api/consumer/pay', { method:'POST', body: JSON.stringify({ bill_document_id: bill.bill_document_id, payment_amount: bill.amount, method_id: selectedId }) });
