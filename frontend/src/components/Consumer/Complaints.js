      ...
      <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 20, padding: 28, width: '100%', maxWidth: 460 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 38, height: 38, borderRadius: 11, background: 'linear-gradient(135deg,#CCFF00,#99CC00)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 3px 10px rgba(204,255,0,0.3)' }}>
              <ComplaintIcon size={19} color="#000" />
            </div>
            <div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button onClick={handleSubmit} disabled={loading} style={{ flex: 1, padding: '11px', borderRadius: 10, border: 'none', background: loading ? t.textMuted : 'linear-gradient(135deg,#CCFF00,#99CC00)', color: '#000', fontSize: 14, fontWeight: 600, fontFamily: fonts.ui, cursor: loading ? 'not-allowed' : 'pointer', boxShadow: loading ? 'none' : '0 4px 14px rgba(204,255,0,0.35)' }}>
...
const ComplaintCard = ({ complaint, t, isDark }) => {
  ...
  return (
    <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 16, overflow: 'hidden', transition: 'box-shadow 0.18s' }}
      onMouseEnter={e => e.currentTarget.style.boxShadow = '0 4px 20px rgba(204,255,0,0.12)'}
      onMouseLeave={e => e.currentTarget.style.boxShadow = 'none'}
    >
      {/* Card header */}
      <div style={{ padding: '18px 20px', display: 'flex', alignItems: 'flex-start', gap: 14 }}>
        <div style={{ width: 38, height: 38, borderRadius: 11, background: util ? util.gradient : 'linear-gradient(135deg,#CCFF00,#99CC00)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          {Icon ? <Icon size={18} color={util ? "#fff" : "#000"} /> : <ComplaintIcon size={18} color="#000" />}
        </div>
          ...
          {timelineSteps.map((step, i) => (
            <div key={step.label} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
              ...
              {i < timelineSteps.length - 1 && (
                <div style={{ position: 'absolute', top: 9, left: '50%', right: '-50%', height: 2, background: timelineSteps[i + 1].done ? 'linear-gradient(90deg,#3B6FFF,#3B6FFF)' : '#1A2235', zIndex: 0 }} />
              )}
              {/* Dot */}
              <div style={{ width: 20, height: 20, borderRadius: '50%', background: step.done ? 'linear-gradient(135deg,#3B6FFF,#00C4FF)' : '#1A2235', border: `2px solid ${step.done ? '#3B6FFF' : '#2A3550'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1, position: 'relative', boxShadow: step.done ? '0 0 8px rgba(59,111,255,0.4)' : 'none', flexShrink: 0 }}>
  ...
  return (
    <div style={{ fontFamily: fonts.ui }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 16 }}>
        ...
        <button onClick={() => setShowModal(true)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '11px 20px', borderRadius: 11, border: 'none', background: 'linear-gradient(135deg,#CCFF00,#99CC00)', color: '#000', fontSize: 14, fontWeight: 600, fontFamily: fonts.ui, cursor: 'pointer', boxShadow: '0 4px 14px rgba(204,255,0,0.35)', whiteSpace: 'nowrap' }}>
          <ComplaintIcon size={17} color="#000" /> File Complaint
        </button>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
        {STATUSES.map(s => (
          <button key={s} onClick={() => setFilter(s)} style={{
            ...
            background: filter === s ? 'rgba(204,255,0,0.08)' : 'transparent',
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '64px 0', color: t.textMuted }}>
          <div style={{ width: 56, height: 56, borderRadius: 16, background: 'linear-gradient(135deg,#CCFF00,#99CC00)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', boxShadow: '0 4px 16px rgba(204,255,0,0.3)' }}>
            <ComplaintIcon size={28} color="#000" />
          </div>
          ...
          {filter === 'All' && (
            <button onClick={() => setShowModal(true)} style={{ padding: '10px 22px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg,#CCFF00,#99CC00)', color: '#000', fontSize: 13, fontWeight: 600, fontFamily: fonts.ui, cursor: 'pointer' }}>
