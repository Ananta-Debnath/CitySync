...
const EyeIcon     = () => <svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7S1 12 1 12z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><circle cx="12" cy="12" r="3" stroke="currentColor" strokeWidth="2"/></svg>;
const EyeOffIcon  = () => <svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M17.94 17.94A10.94 10.94 0 0112 19c-7 0-11-7-11-7a21.77 21.77 0 015.06-5.94M9.9 4.24A10.93 10.93 0 0112 5c7 0 11 7 11 7a21.7 21.7 0 01-3.43 4.36M1 1l22 22" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>;
            ...
            {[{ label: 'Current Password', key: 'current' }, { label: 'New Password', key: 'next' }, { label: 'Confirm Password', key: 'confirm' }].map(f => (
              <div key={f.key} style={{ marginBottom: 14 }}>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  >
                    {showPw[f.key] ? <EyeOffIcon /> : <EyeIcon />}
                  </button>
                </div>
      ...
      <div style={{ background: t.bgCard, border: `2px solid ${isDark ? '#7F1D1D' : '#FCA5A5'}`, borderRadius: 20, padding: 28, width: '100%', maxWidth: 420 }} onClick={e => e.stopPropagation()}>
        ...
        {confirmed && (
          <div style={{ marginBottom: 16 }}>
            ...
            <input type="password" autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Your current password"
...
const Profile = () => {
  ...
  // Inject CSS to hide native password reveal/clear buttons (Chrome/Safari/Edge/IE)
  useEffect(() => {
    if (typeof document === 'undefined') return;
    if (document.getElementById('hide-native-password-icons')) return;
    const s = document.createElement('style');
    s.id = 'hide-native-password-icons';
    s.textContent = `input[type="password"]::-ms-reveal, input[type="password"]::-ms-clear { display: none; }
input[type="password"]::-webkit-credentials-auto-fill-button, input[type="password"]::-webkit-textfield-decoration-button, input[type="password"]::-webkit-password-toggle-button, input[type="password"]::-webkit-clear-button { display: none !important; -webkit-appearance: none; }`;
